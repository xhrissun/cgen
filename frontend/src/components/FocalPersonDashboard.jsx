...
/* ── Design tokens — header + tabs only, sub-components use their own surface ── */
const D = {
  bg: '#0a1628',
  panel: '#0f1e35',
  card: '#152236',
...
function FocalPersonDashboard({ user }) {
  ...
  return (
    <div style={{ minHeight: '100%', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      {/* ── Content — NO dark wrapper, sub-components breathe freely ── */}
      <div>

        {/* OVERVIEW — only this tab uses dark cards */}
        {activeTab === 'overview' && (
          <div style={{ padding: '32px', display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 900 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
              <OverviewCard
                icon={<svg width="20" height="20" fill="none" stroke={D.green} strokeWidth="2" viewBox="0 0 24 24"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>}
                iconBg={D.greenMuted}
              ...
              />
              <OverviewCard
                icon={<svg width="20" height="20" fill="none" stroke={D.blue} strokeWidth="2" viewBox="0 0 24 24"><path d="M16 21v-2a4 4 0 00-4-4H6a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>}
                iconBg={D.blueMuted}
                label="Role"
                value="Focal Person"
              />
              <OverviewCard
                icon={<svg width="20" height="20" fill="none" stroke={D.yellow} strokeWidth="2" viewBox="0 0 24 24"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>}
            ...
            <div style={{ background: D.card, border: `1px solid ${D.border}`, borderRadius: 12 }}>
              <div style={{ padding: '16px 24px', borderBottom: `1px solid ${D.border}` }}>
                <h3 style={{ fontSize: 13, fontWeight: 700, color: D.textPrimary, margin: 0, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Quick Access</h3>
              </div>
              <div style={{ padding: '6px 0' }}>
                {[
                  ...
                  <button key={id} onClick={() => setActiveTab(id)}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 24px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}
            ...
            {/* Info note */}
            <div style={{ background: D.greenMuted, border: `1px solid ${D.greenBorder}`, borderRadius: 12, padding: '16px 20px', display: 'flex', gap: 14 }}>
              <svg width="18" height="18" fill="none" stroke={D.green} strokeWidth="2" viewBox="0 0 24 24" style={{ flexShrink: 0, marginTop: 1 }}><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
              <p style={{ fontSize: 12, color: D.textSecondary, margin: 0, lineHeight: 1.7 }}>
        ...
        {/* All sub-component tabs — no wrapper, just render directly */}
        ...
        {activeTab === 'eodb' && <EODBGenerator userId={user.id || user._id} onDocumentUploaded={handleDocumentUploaded} />}
